...
* `input/` holds a copy of the current [abap2UI5](https://github.com/abap2UI5/abap2UI5) sources. The [update_input](https://github.com/abap2UI5/abap2UI5-local/actions/workflows/update_input.yaml) workflow (dispatched by upstream's `trigger_local` workflow on every push to its `main`, monthly schedule as safety net, or *Run workflow*) overwrites it with the latest upstream version, validates it with [abaplint](https://abaplint.org) (`abaplint.jsonc` points at `/input`) and pushes the refresh to `main`.
